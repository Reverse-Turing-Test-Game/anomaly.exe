...
const questions: Question[] = [
  {
    question: '경제학에서 의사결정을 정량화하기 위해 도입한 것으로, 선택 가능한 대상의 가치값을 일컫는 핵심 단어는 무엇인가?',
    options: ['본능', '지능', '효용', '학습'],
    correct: 2,
  },
  {
    question: '생물학적 신경세포를 단순화하여 모사한 것으로, 인공신경망을 구성하는 가장 기초적인 단위를 가리키는 단어는 무엇인가?',
    options: ['시냅스', '엑손', '뉴런', '퍼셉트론'],
    correct: 3,
  },
  {
    question: '샐리-앤 검사(Sally-Anne test)를 통해 주로 측정하고자 하는 것으로, 타인의 생각이나 의도, 정신 상태를 스스로의 것과 구별하여 추론하고 이해하는 능력을 일컫는 단어는 무엇인가?',
    options: ['학습이론', '마음이론', '게임이론', '효용이론'],
    correct: 1,
  },
];
...
export function Level2() {
  ...
  const handleAnswer = (index: number) => {
    ...
    // 비동기 처리
    setTimeout(async () => {
      // 현재 맞춘 개수에 이번 정답 여부를 더해 진짜 최종 점수 미리 계산
        ...
        try {
          ...
          const res = await fetch(`${API_URL}/game/mission/submit`, {
            ...
            body: JSON.stringify({
              ...
              missionId: 'level2', 
              inputData: { 
                  correctCount: nextCorrectCount, // 방금 계산한 점수
                  tryCount: 1 
              }
            })
          });
