...
export function Level5() {
  ...
  const [answers, setAnswers] = useState<string[]>([]); // 3개 답변 누적
  ...
  // ── 판정 처리 ──────────────────────────────────────────────────────────────
  const handleVerdict = (ariaText: string) => {
    setPhase('judging');
    setRound(4);
    setTimeout(() => {
  ...
  const sendMessage = async () => {
    ...
    const newAnswers = [...answers, input.trim()]; // 답변 누적
    ...
    try {
      if (isOffline) {
        ...
        // ── 1, 2라운드: 다음 질문만 표시 ─────────────────────────────────
        const nextQuestion = FIXED_QUESTIONS[round]; // round=1 → index 1
        ...
        const res = await fetch(`${API_URL}/game/mission/submit`, {
          ...
          body: JSON.stringify({
            ...
            missionId: 'level5',
            inputData: {
              answers: newAnswers, // ["답변1", "답변2", "답변3"]
            },
          }),
        });
        ...
        <div className="mb-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <motion.button
              ...
              className="flex items-center gap-1 px-3 py-1 border border-[#ff00ff] text-[#ff00ff] font-mono text-xs hover:bg-[#ff00ff] hover:text-[#000814] transition-colors"
              style={{ boxShadow: '0 0 8px rgba(255,0,255,0.3)' }}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              title="API 연결 없이 다음 레벨로"
            >
