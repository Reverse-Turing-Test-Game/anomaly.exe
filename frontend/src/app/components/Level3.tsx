...
function getInterval(floor: number): number {
  ...
  if (distToTarget >= 5) return 200;
  if (distToTarget === 4) return 200;
  if (distToTarget === 3) return 180;
  if (distToTarget === 2) return 140;
  if (distToTarget === 1) return 120;
  return 120; 
...
export function Level3() {
  ...
  const stopElevator = useCallback(async () => { // async 추가
