...
export function Level1() {
 ...
 // async(비동기: 서버에 데이터를 요청하고 응답이 올 때까지 멈추지 않고 기다려주는 자바스크립트 키워드)를 붙여줍니다.
  ...
  const endDraw = async () => {
    ...
    try {
      // 1. 주머니(localStorage)에서 로그인 시 저장한 진짜 세션 ID를 꺼냅니다.
      ...
      // 3. 서버가 "성공(isSuccess: true)"이라고 응답했을 때만 통과
      // 팀장님 API 구조에 따라 data.data.isSuccess 또는 data.isSuccess 확인
