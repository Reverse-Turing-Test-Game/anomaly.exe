...
async function fetchTop3(): Promise<RankEntry[]> {
  const res = await fetch(`${API_URL}/ranking/top3`);
  if (!res.ok) throw new Error('failed');
  const json = await res.json();
  // 백엔드가 { top3: [...] } 형태로 주는지, 바로 배열 [...] 로 주는지에 따라 유연하게 대처
  return json.data.top3 || json.data; 
}

// ★ 2. sessionId 대신 nickname을 받아서 서버로 보내도록 수정!
async function fetchMyRank(nickname: string): Promise<MyRank | null> {
  const res = await fetch(`${API_URL}/ranking/my?nickname=${encodeURIComponent(nickname)}`);
...
export function Ranking() {
  ...
  const loadRanking = async () => {
    ...
    try {
      // 1. TOP 3 가져오기
      const top3Data = await fetchTop3();
      setTop3(top3Data);

      // ★ 2. 로컬스토리지에 저장된 nickname을 꺼내서 내 순위 가져오기
      if (player.nickname) {
        const myRankData = await fetchMyRank(player.nickname);
