...
export function StoryPage1() {
  ...
  // 닉네임을 저장할 상태 변수 (처음엔 LOADING...으로 표시)
  const [nickname, setNickname] = useState<string>('LOADING...');

  // 화면이 처음 켜질 때 백엔드에 닉네임을 물어보는 로직
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // 이미 만들어져 있는 getSession API 호출
        const res = await fetch(`${API_URL}/game/${currentSessionId}`);
        
        if (res.ok) {
          const result = await res.json();
          // 백엔드가 준 데이터에서 닉네임만 쏙 빼서 화면에 적용
        ...
        setNickname('AGENT'); // 에러 나면 기본 요원 이름으로 대체
      }
    };
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[#5de2e7] text-lg font-mono leading-relaxed">
                    <span className="text-[#ff00ff]">&gt;</span> You are{' '}
                    {/* ★ 하드코딩 되어있던 "nickname"을 상태 변수 {nickname}으로 교체! */}
                    <span className="text-[#00ff00]">{nickname}</span>, 
