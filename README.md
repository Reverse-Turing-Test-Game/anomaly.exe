...
```text
/ (Repository Root)
├── backend/     # Node.js + Express + Firebase Admin 백엔드 서버
└── frontend/         # React + Vite + Tailwind CSS 프론트엔드 앱
...
cd backend
npm install
npm install express firebase-admin @google/generative-ai dotenv cors morgan axios
npm install -D nodemon

package.json 스크립트 설정 (package.json 파일의 scripts 부분을 아래와 같이 수정)
"scripts": {
...
🔒 환경 변수 설정
backend 폴더 루트에 .env 파일을 생성하고 아래 내용을 입력합니다.
